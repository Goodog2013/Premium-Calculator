...
export function WindowTitleBar({ subtitle }: WindowTitleBarProps) {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 rounded-2xl bg-[linear-gradient(140deg,rgb(var(--accent-rgb)/0.14),transparent_45%)] dark:bg-[linear-gradient(140deg,rgb(var(--accent-rgb)/0.2),transparent_45%)]" />

      <div
        className="relative flex items-center gap-2"
        data-tauri-drag-region={isTauriEnvironment() ? true : undefined}
      >
        <span className="accent-dot inline-flex h-2.5 w-2.5 animate-pulse rounded-full" />
      ...
      <div className="relative flex items-center gap-1">
        <button
          ...
          className={cn(
            ...
            'accent-border-hover accent-soft-hover hover:text-slate-900',
